...
# ╔═╡ b68eaef7-6d6c-419b-82ff-0818a53456d7
begin
	for pkg in ("Mods", "Karnak")
		push!(LOAD_PATH, expanduser("~/.julia/dev/$pkg"))
	end
	for pkg in ("Bach", "TimeUnits")
		push!(LOAD_PATH, expanduser("~/.julia/dev/$pkg"))
	end
end

# ╔═╡ 90a8aef2-3c1f-11f0-0510-0b827aecea25
begin
	using Test
	using MIDI
...
# ╔═╡ c9e2aa97-7daa-4fac-a762-222a3382a1ff
begin
	using PlutoLinks
	using Reexport, Colors, NetworkLayout, Luxor
end

# ╔═╡ d70cd84b-da6f-4d49-a3f6-a624039d760c
@revise using Bach

# ╔═╡ abd6f520-446e-42b5-a084-b7be2a9acf3d
begin
	g = SimpleDiGraph(TYPE_VERTEX(5))
	add_edge!(g, 1, 2)
	add_edge!(g, 1, 4)
	add_edge!(g, 2, 4)
...
# ╔═╡ 6066ade0-afba-4b83-95ff-2846d2dacbed
draw_graph(g)

# ╔═╡ d5a33595-91f2-4de2-86f4-f7932cb2abc8
animate_graph(g; width = 250.0, framerate = 2)
...
manifest_format = "2.0"
project_hash = "db3a8dfcecdefd077bbbd1f785385cece485985a"
